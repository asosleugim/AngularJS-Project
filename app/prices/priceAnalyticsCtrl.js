...
(function () {
    ...
    function PriceAnalyticsCtrl($scope, $filter, products, productService) {
        ...
        // Calculates the marginPercent and marginAmount variables
        for (var i = 0; i < products.length; i++) {
            products[i].marginPercent = productService.calculateMarginFromPercent(products[i].price, products[i].cost);
            products[i].marginAmount = productService.calculateMarginFromAmount(products[i].price, products[i].cost);
        }
